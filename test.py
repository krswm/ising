...
def get_ticks(min, max):
    ...
    for new_exp, new_man in [(exp, 2.5), (exp, 5.0), (exp, 10.0), (exp, 25.0)]:
        if man <= new_man:
            inc = new_man * 10.0 ** new_exp / 5.0
            new_min = (min // inc) * inc
            new_max = new_man * 10.0 ** new_exp + new_min
            if new_max < max:
                continue
            return [new_man * 10.0 ** new_exp * i / 5.0 + new_min for i in range(6)]
